...
v<-visite_stagione_GenFeb
# v<-v[,-1]
rm(visite_stagione_GenFeb)
...
rm(piattaforma_stagione_GenFeb)
# p<-p[,-1]
...
pg<-pagine_stagione_GenFeb
rm(pagine_stagione_GenFeb)
...
#trattengo da pg solo pages, entry_pages e visitorID
pg1<-pg[,c("visitorID", "pages", "entry_pages")]

pg1<-merge(pg1, URL, by.x="pages",by.y="pages")
...
#load(file=paste0(dir_output,"/entryURL_checkpoint"))

save(file=paste0(dir_output,"/cookies_checkpoint_pagine_4"), cookies)
#load(file=paste0(dir_output,"/cookies_checkpoint_pagine_4"))
# pg<-left_join(pg, entryURL, by="entry_pages")

pg1<-pg[,c("visitorID", "pages", "entry_pages")]
pg1<-merge(pg1, entryURL, by.x="entry_pages",by.y="entry_pages")

save(file=paste0(dir_output,"/pg_mergeentrtyURL"), pg1)

PrimaCol<-which(names(pg1)=="pages_AreaAssistenza")
t<-aggregate(pg1[PrimaCol:ncol(pg1)], by=list(factor(pg1$visitorID)), sum)
names(t)[1]<-"visitorID"
t$visitorID<-as.character(t$visitorID)
t[is.na(t)]<-0
cookies<-left_join(cookies,t, by="visitorID")
...
pd<-prodotti_stagione_GenFeb


rm(prodotti_stagione_GenFeb)
#operazioni fatte una tantum con system locale ENGLISH
...
log<-log_stagione_GenFeb
rm(log_stagione_GenFeb)
