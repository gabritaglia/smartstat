...
for(i in 1:length(soggetti)){
  ...
  levels(subset$outcome)[levels(subset$outcome)=="carrello"]<-"naviga"
  levels(subset$outcome)<-c(0,1) #naviga compra
  ...
  modelliEnzoNA[[i]]<-modello
  names(modelliEnzoNA)[i]<-soggetti[i]
  
}


save(modelliEnzoNA, file="output/modelliEnzoNA")
